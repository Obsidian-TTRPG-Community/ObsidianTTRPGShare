---
statblock: inline
tags: monster
name: Bore Worm Monarch
---
```statblock
layout: Basic Pathfinder 1e Layout
source: "Pathfinder No. 117: Assault on Longshadow"
Monster_CR: 7
name: Bore Worm Monarch
Monster_XP: 3200
alignment: N
size: Large
type: vermin
INI: -2
senses: tremorsense
AC: 19, touch 7, flat-footed 19 (dex -2, natural +12, size -1)
HP: 78
HD: 12d8+24
saves: Fort +10, Ref +2, Will +4
immune: acid, mind-affecting effects
DR: 10/piercing
weak: vulnerable to water
speed: 20 ft., burrow 40 ft.
melee: bite +13 (2d6+7 plus 2d6 acid and painful bite)
special_attacks: painful bite (DC 18)
space: 10 ft.
reach: 10 ft.
pf1e_stats: [20, 7, 15, None, 10, 2]
BAB: 9
CMB: 15
CMD: 23 (can’t be tripped)
skills: 
special_qualities: compression, corrosive wake
ecology:
  - name: Environment
    desc: any underground
  - name: Organisation
    desc: solitary, pair, or tremor (1 plus 1d6 bore worm swarms)
  - name: Treasure
    desc: none
special_abilities:
  - name: Corrosive Wake (Ex)
    desc: When a bore worm monarch burrows, it can move through solid material, including stone and metal, as easily as a fish swims through water. It leaves behind a 5-foot-diameter, smooth-sided tunnel coated in acid that deals 1d3 points of acid damage to creatures coming into contact with its surface. This acid lasts a number of rounds equal to the hardness of material it burrowed through.

A single, finger-length bore worm has never been much of a threat to any one creature, but isolated bore worms are found only in laboratories. Bore worm swarms, as they are more commonly encountered, consume organic material with a voracious appetite, tunneling through the earth like water as they reach progressively larger sizes.
sources:
  - name: Pathfinder No. 117: Assault on Longshadow
    desc: 82
desc_short: Bright red and coated in an opaque yellow slime, this monstrous worm has a menacing pair of barbed mandibles.
```
# Description

# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Bore%20Worm%20Monarch)
```encounter-table
name: Bore Worm Monarch
creatures:
  - 1: Bore Worm Monarch
```
