---
statblock: inline
tags: monster
name: NPC Elven Recluse
---
```statblock
layout: Basic Pathfinder 1e Layout
source: "NPC Codex"
Monster_CR: 16
name: NPC Elven Recluse
Monster_XP: 76800
race: Elf
class: fighter 17
alignment: N
size: Medium
type: humanoid
subtype: (elf)
INI: +5
perception: +3
senses: low-light vision
AC: 35, touch 18, flat-footed 28 (armor +11, deflection +1, dex +5, dodge +2, natural +5, shield +1)
HP: 149
HD: 17d10+51
saves: Fort +14, Ref +14, Will +11
saves_other: +2 vs. enchantments, +4 vs. fear
immune: bleed (hit points only), sleep
resist: cold 10, fire 10
defensive_abilities: bravery +4
speed: 60 ft.
melee: mwk quarterstaff +26/+26/+21/+16/+11 (1d6+11), mwk quarterstaff +26/+21/+16 (1d6+9)
ranged: +1 seeking composite longbow +26/+26/+26/+21/+16/+11 (1d8+9/×3)
special_attacks: weapon training (double +4, bows +3, spears +2, light blades +1)
space: 5 ft.
reach: 5 ft. (10 ft. with longspear)
tactics:
  - name: Before Combat
    desc: The fighter drinks her potion of barkskin and both potions of resist energy. She then activates her boots of speed.
  - name: During Combat
    desc: The fighter chooses her weapon based on the weaknesses of her foes. She engages slower foes with her bow and keeps her distance. Against lightly armored enemies or spellcasters, she fights with a shillelagh-enhanced quarterstaff.
  - name: Base Statistics
    desc: Without barkskin, resist energy, and haste from her boots of speed, the fighter’s statistics are AC 29, touch 17, flat-footed 23; Resist -; Ref +13; Speed 30 ft.; Melee mwk quarterstaff +25/+20/+15/+10 (1d6+11), mwk quarterstaff +25/+20/+15 (1d6+9) or longspear +22/+17/+12/+7 (1d8+6/×3) or kukri +21/+16/+11/+6 (1d4+4/18-20); Ranged +1 seeking composite longbow +25/+25/+20/+15/+10 (1d8+9/×3); CMB +20.
pf1e_stats: [17, 20, 12, 13, 12, 8]
BAB: 17
CMB: 21
CMD: 38
feats: Combat Expertise, Dodge, Greater Two-Weapon Fighting, Greater Weapon Focus (quarterstaff), Greater Weapon Specialization (quarterstaff), Improved Two-Weapon Fighting, Iron Will, Lunge, Point-Blank Shot, Power Attack, Rapid Shot, Toughness, Two-Weapon Defense, Two-Weapon Fighting, Weapon Focus (composite longbow), Weapon Focus (quarterstaff), Weapon Specialization (composite longbow), Weapon Specialization (quarterstaff)
skills: Climb +10, Craft (bows) +5, Craft (weapons) +5, Knowledge (geography) +11, Knowledge (nature) +16, Perception +3, Stealth +10, Survival +16, Swim +6
languages: Common, Elven, Sylvan
special_qualities: armor training 4, elven magic, weapon familiarity
gear:
  - name: combat
    desc: oil of shillelagh (CL 5th) (3), potion of barkskin (CL 12th), potion of resist energy (cold), potion of resist energy (fire), restorative ointment
  - name: other
    desc: +2 full plate, +1 seeking composite longbow with 40 arrows, kukri, longspear, masterwork quarterstaff, belt of giant strength +4, boots of speed, cloak of resistance +3, periapt of wound closure, ring of protection +1, artisan’s tools, ink (2), inkpens (2), journal (as spellbook), 48 gp
sources:
  - name: NPC Codex
    desc: 92
```
# Description
These elves shun civilization and protect ancient groves.
# Source Link
[Archives of Nethys](https://aonprd.com/NPCDisplay.aspx?ItemName=Elven%20Recluse)
```encounter-table
name: NPC Elven Recluse
creatures:
  - 1: NPC Elven Recluse
```
