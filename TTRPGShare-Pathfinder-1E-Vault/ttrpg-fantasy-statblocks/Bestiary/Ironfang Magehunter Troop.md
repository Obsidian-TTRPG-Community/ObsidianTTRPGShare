---
statblock: inline
tags: monster
name: Ironfang Magehunter Troop
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "Pathfinder No. 120: Vault of the Onyx Citadel"
Monster_CR: 14
name: Ironfang Magehunter Troop
Monster_XP: 38400
alignment: LE
size: Medium
type: humanoid
subtype: (goblinoid, troop)
INI: +9
perception: +12
senses: darkvision
AC: 28, touch 15, flat-footed 23 (armor +5, dex +5, natural +8)
HP: 199
HP_extra: 
HD: 19d8+114
saves: Fort +16, Ref +13, Will +11
defensive_abilities: stalwart, troop traits
speed: 30 ft.
melee: troop (4d6+3 plus distraction)
special_attacks: arcane hunter, distraction (DC 24), magic weapons, volley
space: 20 ft.
reach: 5 ft.
pf1e_stats: [17, 20, 20, 11, 16, 11]
BAB: 14
CMB: 17
CMD: 32
feats: Combat Reflexes, Diehard, Disruptive, Endurance, Improved Initiative, Iron Will, Lightning Reflexes, Spellbreaker, Step Up, Toughness
skills: Knowledge (arcana) +10, Perception +12, Spellcraft +19, Stealth +9
racial_modifiers:
- Stealth 4
languages: Common, Goblin
ecology:
  - name: Environment
    desc: any
  - name: Organisation
    desc: solitary, pair, or division (3-6)
    desc: standard
special_abilities:
  - name: Arcane Hunter (Ex)
    desc: A magehunter troop deals an additional 2d6 points of damage with its troop attack to creatures with levels of any arcane spellcasting class or that use spell-like abilities not derived from a divine spellcasting class.
  - name: Magic Weapons (Ex)
    desc: A magehunter troop’s attacks count as magic for the purpose of bypassing damage reduction.
  - name: Stalwart (Ex)
    desc: When a magehunter troop succeeds at a Fortitude or Will saving throw against a spell or spell-like ability that has a partial effect even on a successful save, the troop is completely unaffected by it.
  - name: Volley (Ex)
    desc: A magehunter troop can fire a volley of crossbow bolts as a standard action. This attack takes the form of up to four lines with a range of 100 feet. These lines can start from the corner of any square in the troop’s space. All creatures in any of these lines take 4d8 points of piercing damage (Reflex DC 24 half). The save DC is Dexterity-based.
sources:
  - name: Pathfinder No. 120: Vault of the Onyx Citadel
    desc: 88
desc_short: This grim band of hobgoblin hunters is equipped with sturdy armor and well-used weapons.
```
# Description
Most hobgoblins in this troop are equipped with +1 mithral chain shirts, +1 scimitars, and +1 heavy crossbows.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Ironfang%20Magehunter%20Troop)
```encounter-table
name: Ironfang Magehunter Troop
creatures:
  - 1: Ironfang Magehunter Troop
```
