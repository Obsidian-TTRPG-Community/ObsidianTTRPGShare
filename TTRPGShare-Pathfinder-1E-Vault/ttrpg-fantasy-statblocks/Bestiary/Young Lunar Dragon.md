---
statblock: inline
tags: monster
name: Young Lunar Dragon
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "Bestiary 4"
Monster_CR: 9
name: Young Lunar Dragon
Monster_XP: 6400
alignment: CN
size: Large
type: dragon
INI: +0
perception: +17
senses: dragon senses, see in darkness
AC: 21, touch 9, flat-footed 21 (natural +12, size -1)
HP: 105
HP_extra: 
HD: 10d12+40
saves: Fort +10, Ref +7, Will +11
immune: cold, paralysis, sleep
defensive_abilities: reflected light
speed: 40 ft., fly 200 ft. (poor), burrow 20 ft.
melee: bite +14 (2d6+7), 2 claws +14 (1d8+5), 2 wings +12 (1d6+2), tail slap +12 (1d8+7)
special_attacks: bewildering breath, breath weapon (80-ft. line, 6d8 cold, DC 18), moonsilver
space: 10 ft.
reach: 5 ft. (10 ft. with bite)
pf1e_stats: [21, 11, 16, 16, 18, 17]
BAB: 10
CMB: 16
CMD: 26 (30 vs. trip)
feats: Flyby Attack, Multiattack, Power Attack, Step Up, Toughness
skills: Diplomacy +16, Fly +7, Intimidate +16, Knowledge (arcana) +10, Knowledge (geography) +10, Knowledge (history) +10, Knowledge (local) +10, Knowledge (nature) +10, Perception +17, Sense Motive +17, Spellcraft +16, Use Magic Device +16
languages: Aklo, Auran, Common, Draconic
special_qualities: no breath, starflight
ecology:
  - name: Environment
    desc: vacuum
  - name: Organisation
    desc: solitary
    desc: triple
known_spells:
  - name:
    desc: (CL 1)
  - name: 1st (4/day)
    desc: comprehend languages, mage armor
  - name: 0 (at-will)
    desc: detect magic, light, mage hand, read magic
spell-like_abilities:
  - name:
    desc: (CL 10; Concentration +13)
  - name: At will
    desc: dancing lights
sources:
  - name: Bestiary 4
    desc: 66
desc_short: A shimmering halo surrounds this regal dragon’s spine-crowned head, and its vast wings ripple like milky gossamer lit by the moon.
```
# Description
Lunar dragons frequently interact with mortals, spending long hours watching the activities occurring on planets that interest them.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Young%20Lunar%20Dragon)
```encounter-table
name: Young Lunar Dragon
creatures:
  - 1: Young Lunar Dragon
```
