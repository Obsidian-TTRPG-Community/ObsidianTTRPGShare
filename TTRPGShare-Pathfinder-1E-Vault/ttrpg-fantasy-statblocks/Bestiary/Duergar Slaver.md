---
statblock: inline
tags: monster
name: Duergar Slaver
---
```statblock
layout: Basic Pathfinder 1e Layout
source: "Monster Codex"
Monster_CR: 1/2
name: Duergar Slaver
Monster_XP: 200
race: Duergar
class: rogue 1
alignment: LE
size: Medium
type: humanoid
subtype: (dwarf)
INI: +6
perception: +7
senses: darkvision
AC: 15, touch 12, flat-footed 13 (armor +3, dex +2)
HP: 10
HD: 1d8+2
saves: Fort +1, Ref +4, Will +3
saves_other: +2 vs. spells
immune: paralysis, phantasms, poison
weak: light sensitivity
speed: 20 ft.
melee: short sword +1 (1d6+1/19-20)
ranged: mwk light crossbow +3 (1d8/19-20)
special_attacks: sneak attack +1d6
pf1e_stats: [13, 15, 12, 12, 16, 4]
BAB: 0
CMB: 1
CMD: 13 (17 vs. bull rush or trip)
feats: Improved Initiative
skills: Acrobatics +5, Climb +4, Disable Device +5, Escape Artist +5, Knowledge (dungeoneering) +5, Perception +7, Sense Motive +7, Stealth +5, Survival +4
languages: Common, Dwarven, Undercommon
special_qualities: slow and steady, stability, trapfinding +1
gear:
  - name: gear
    desc: studded leather, mwk light crossbow with 20 bolts, sap, short sword, 18 gp
ecology:
  - name: Environment
    desc: any underground
spell-like_abilities:
  - name:
    desc: (CL 1; Concentration -2)
  - name: 1/day
    desc: enlarge person, invisibility
sources:
  - name: Monster Codex
    desc: 47
```
# Description
Duergar rogues primarily capture and discipline slaves.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Duergar%20Slaver)
```encounter-table
name: Duergar Slaver
creatures:
  - 1: Duergar Slaver
```
