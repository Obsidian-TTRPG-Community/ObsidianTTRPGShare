---
statblock: inline
tags: monster
name: NPC Freelance Thief
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "NPC Codex"
Monster_CR: 6
name: NPC Freelance Thief
Monster_XP: 2400
race: Human
class: rogue 7
alignment: NE
size: Medium
type: humanoid
subtype: (human)
INI: +4
perception: +10
AC: 19, touch 15, flat-footed 14 (armor +4, dex +4, dodge +1)
HP: 42
HP_extra: 
HD: 7d8+7
saves: Fort +4, Ref +10, Will +3
defensive_abilities: evasion, trap sense +2, uncanny dodge
speed: 30 ft.
melee: +1 rapier +10 (1d6+3/18-20)
ranged: mwk light crossbow +10 (1d8/19-20)
special_attacks: sneak attack +4d6
tactics:
  - name: During Combat
    desc: The rogue uses Spring Attack and Combat Expertise.
pf1e_stats: [14, 18, 12, 13, 10, 8]
BAB: 5
CMB: 7
CMD: 22
feats: Combat Reflexes, Dodge, Mobility, Spring Attack, Weapon Finesse
skills: Acrobatics +14, Bluff +9, Climb +12, Diplomacy +9, Disable Device +14, Disguise +9, Knowledge (local) +11, Perception +10, Sense Motive +10, Sleight of Hand +14, Stealth +14
languages: Common, Halfling
special_qualities: rogue talents (bleeding attack +4, fast stealth, stand up), trapfinding +3
gear:
  - name: combat
    desc: potion of cure moderate wounds, potion of invisibility, tanglefoot bag
  - name: other
    desc: +1 studded leather, +1 rapier, masterwork light crossbow with 20 bolts, cloak of resistance +1, thieves’ tools, 489 gp
sources:
  - name: NPC Codex
    desc: 147
```
# Description
The freelance thief sells his loyalty for gold.
# Source Link
[Archives of Nethys](https://aonprd.com/NPCDisplay.aspx?ItemName=Freelance%20Thief)
```encounter-table
name: NPC Freelance Thief
creatures:
  - 1: NPC Freelance Thief
```
