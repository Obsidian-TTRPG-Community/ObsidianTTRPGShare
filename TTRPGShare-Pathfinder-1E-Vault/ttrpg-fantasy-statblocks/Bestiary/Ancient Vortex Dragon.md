---
statblock: inline
tags: monster
name: Ancient Vortex Dragon
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "Bestiary 4"
Monster_CR: 19
name: Ancient Vortex Dragon
Monster_XP: 204800
alignment: LN
size: Gargantuan
type: dragon
subtype: (fire)
INI: +4
perception: +34
senses: dragon senses, see in darkness
aura: alien presence
AC: 39, touch 6, flat-footed 39 (natural +33, size -4)
HP: 337
HP_extra: 
HD: 25d12+175
saves: Fort +23, Ref +16, Will +20
immune: fire, paralysis, sleep
DR: 15/magic
SR: 30
weak: vulnerable cold
speed: 60 ft., fly 250 ft. (good)
melee: bite +33 (4d6+18/19-20), 2 claws +33 (2d8+12/19-20), 2 wings +31 (2d6+6), tail slap +31 (2d8+18)
special_attacks: breath weapon (60-ft. cone, 20d8 fire, DC 29), collapsing breath (DC 29), crush, fragmented strike, tail sweep
space: 20 ft.
reach: 15 ft. (20 ft. with bite)
pf1e_stats: [35, 10, 25, 24, 23, 24]
BAB: 25
CMB: 41
CMD: 51 (55 vs. trip)
feats: Critical Focus, Flyby Attack, Great Fortitude, Improved Critical (bite), Improved Critical (claw), Improved Initiative, Lightning Reflexes, Multiattack, Power Attack, Quicken Spell, Snatch, Staggering Critical, Wingover
skills: Bluff +35, Diplomacy +35, Fly +26, Intimidate +35, Knowledge (arcana) +35, Knowledge (geography) +35, Knowledge (history) +35, Knowledge (nature) +35, Knowledge (planes) +35, Knowledge (religion) +35, Perception +34, Sense Motive +34, Spellcraft +35
languages: Abyssal, Auran, Celestial, Common, Draconic, Dwarven, Elven, Infernal
special_qualities: agile, galactic emissary, no breath, starflight
ecology:
  - name: Environment
    desc: vacuum
  - name: Organisation
    desc: solitary
    desc: triple
known_spells:
  - name:
    desc: (CL 15)
  - name: 7th (5/day)
    desc: forcecage (DC24), spell turning
  - name: 6th (7/day)
    desc: forceful hand, shadow walk (DC23), true seeing
  - name: 5th (7/day)
    desc: dream, passwall, telekinesis, wall of force
  - name: 4th (7/day)
    desc: fear (DC21), scrying (DC21), solid fog, stoneskin
  - name: 3rd (8/day)
    desc: dispel magic, haste, nondetection, tongues
  - name: 2nd (8/day)
    desc: cat’s grace, mirror image, obscure object, resist energy, scorching ray
  - name: 1st (8/day)
    desc: feather fall, identify, mage armor, magic missile, true strike
  - name: 0 (at-will)
    desc: detect magic, light, message, read magic, 5 more
spell-like_abilities:
  - name:
    desc: (CL 25; Concentration +32)
  - name: At will
    desc: anticipate peril, dimension door, entropic shield, greater teleport
sources:
  - name: Bestiary 4
    desc: 75
```
# Description
Vortex dragons are the undisputed masters of travel across the galaxies, serving as messengers and observers for the greatest outer dragons and godlike entities in the great beyond.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Ancient%20Vortex%20Dragon)
```encounter-table
name: Ancient Vortex Dragon
creatures:
  - 1: Ancient Vortex Dragon
```
