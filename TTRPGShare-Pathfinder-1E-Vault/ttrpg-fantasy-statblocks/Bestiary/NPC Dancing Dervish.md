---
statblock: inline
tags: monster
name: NPC Dancing Dervish
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "NPC Codex"
Monster_CR: 9
name: NPC Dancing Dervish
Monster_XP: 6400
race: Human
class: rogue 10
alignment: N
size: Medium
type: humanoid
subtype: (human)
INI: +8
perception: +12
AC: 20, touch 16, flat-footed 15 (armor +4, deflection +1, dex +4, dodge +1)
HP: 68
HP_extra: 
HD: 10d8+20
saves: Fort +4, Ref +11, Will +2
defensive_abilities: evasion, improved uncanny dodge, trap sense +3
speed: 30 ft.
melee: +1 keen rapier +13/+8 (1d6+3/15-20)
ranged: light crossbow +11 (1d8/19-20)
special_attacks: sneak attack +5d6
tactics:
  - name: Before Combat
    desc: The rogue relishes combat, but often prefers to hide the full extent of his abilities until he’s completely surrounded, the better to take his enemies by surprise and make use of Whirlwind Attack.
  - name: During Combat
    desc: The rogue charges into combat, trying to get in the middle of as many enemies as possible before using Whirlwind Attack, laughing and singing prayers as he lays waste to his enemies.
pf1e_stats: [14, 19, 12, 13, 8, 10]
BAB: 7
CMB: 9
CMD: 25
feats: Combat Expertise, Dodge, Improved Initiative, Mobility, Nimble Moves, Spring Attack, Weapon Finesse, Weapon Focus (rapier), Whirlwind Attack
skills: Acrobatics +17, Climb +15, Disable Device +19, Escape Artist +17, Intimidate +13, Perception +12, Perform (dance) +13, Sleight of Hand +17, Stealth +17, Swim +15
languages: Common, Elven
special_qualities: rogue talents (combat trick, finesse rogue, opportunist, surprise attack, weapon training), trapfinding +5
gear:
  - name: combat
    desc: potion of blur, potion of cat’s grace, alchemist’s fire (2)
  - name: other
    desc: +1 studded leather, +1 keen rapier, light crossbow with 20 bolts, ring of protection +1, masterwork thieves’ tools, 480 gp
sources:
  - name: NPC Codex
    desc: 149
```
# Description
The dancing dervish spins as a way to focus himself on achieving harmony with the gods.
# Source Link
[Archives of Nethys](https://aonprd.com/NPCDisplay.aspx?ItemName=Dancing%20Dervish)
```encounter-table
name: NPC Dancing Dervish
creatures:
  - 1: NPC Dancing Dervish
```
