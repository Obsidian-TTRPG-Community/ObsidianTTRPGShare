---
statblock: inline
tags: monster
name: Sahuagin Scout
---
```statblock
layout: Basic Pathfinder 1e Layout
source: "Monster Codex"
Monster_CR: 3
name: Sahuagin Scout
Monster_XP: 800
race: Sahuagin
class: rogue 2
alignment: LE
size: Medium
type: monstrous humanoid
subtype: (aquatic)
INI: +3
perception: +8
senses: blindsense, darkvision
AC: 18, touch 13, flat-footed 15 (dex +3, natural +5)
HP: 34
HP_extra: HD 4
HD: 2d10+2d8+14
saves: Fort +3, Ref +9, Will +4
defensive_abilities: evasion
weak: light blindness
speed: 30 ft., swim 60 ft.
melee: mwk trident +8 (1d8+6), bite +2 (1d4+2)
ranged: mwk underwater heavy crossbow +7 (1d10/19-20)
special_attacks: blood frenzy, sneak attack +1d6
tactics:
  - name: During Combat
    desc: The scout shoots the appropriate bane bolts before closing to melee.
pf1e_stats: [18, 17, 16, 17, 12, 7]
BAB: 3
CMB: 7
CMD: 20
feats: Combat Expertise, Rapid Reload (heavy crossbow)
skills: Acrobatics +10, Climb +11, Disable Device +12, Escape Artist +10, Knowledge (geography) +7, Perception +8, Ride +7, Sense Motive +8, Stealth +10, Survival +7, Swim +12
languages: Aquan, Common, Elven, speak with sharks
special_qualities: rogue talents (bleeding attack +1), trapfinding +1
gear:
  - name: combat
    desc: +1 elf-bane bolts (3), +1 human-bane bolts (3), +1 merfolk-bane bolts (3), potion of cure moderate wounds
  - name: other
    desc: mwk trident, mwk underwater heavy crossbow with 20 bolts, feather token (anchor), dagger, thieves’ tools, 53 gp
ecology:
  - name: Environment
    desc: temperate or warm ocean
sources:
  - name: Monster Codex
    desc: 190
```
# Description
Agents lead the way for other sahuagin by scouting enemy patrols and hidden fortifications.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Sahuagin%20Scout)
```encounter-table
name: Sahuagin Scout
creatures:
  - 1: Sahuagin Scout
```
