---
statblock: inline
tags: monster
name: Mythic Locust Plague Swarm
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "Bestiary 5"
Monster_CR: 15
name: Mythic Locust Plague Swarm
Monster_XP: 51200
alignment: NE
size: Fine
type: magical beast
subtype: (mythic, swarm)
INI: +16/-4;  dual initiative
perception: +24
senses: darkvision, low-light vision
AC: 31, touch 23, flat-footed 26 (dex +4, dodge +1, natural +8, size +8)
HP: 216
HP_extra: fast healing 5
HD: 21d10+101
saves: Fort +15, Ref +18, Will +13
saves_other: second save
immune: mind-affecting effects, weapon damage
DR: 10/epic
defensive_abilities: reform, swarm traits, unstoppable
speed: 10 ft., fly 40 ft. (good), climb 10 ft.
melee: swarm (5d6 plus 5d6 blight, devour, and distraction)
special_attacks: devour, distraction (DC 23), mythic power (8/day, surge +1d10)
space: 20 ft.
reach: 0 ft.
pf1e_stats: [6, 19, 12, 5, 18, 9]
BAB: 21
CMB: None
CMD: None
feats: Ability Focus (distraction), Dodge, Extra Mythic Power, Great Fortitude, Improved Initiative, Improved Lightning Reflexes, Iron Will, Lightning Reflexes, Lightning Stance, Skill Focus (Fly), Skill Focus (Perception), Wind Stance
skills: Climb +12, Fly +35, Perception +24
languages: Abyssal (can’t speak)
special_qualities: multiply (locust swarm with the devour ability)
special_abilities:
  - name: Devour (Ex)
    desc: A locust plague swarm devours everything in its path. Its swarm attack damages all unattended objects and all attended items that are not magical. Each round, the locusts also damage one attended magic item worn or carried by each creature damaged by the swarm. Each round, the swarm can affect only one magic item per creature, even with its dual initiative. If a creature possesses more than one attended magic item, use the table for items affected by magical attacks (Pathfinder RPG Core 216) to Rulebook determine which magic item the swarm targets. The locusts’ powerful jaws count as adamantine for the purpose of bypassing DR and bypass all hardness less than 20.
  - name: Reform (Su)
    desc: A locust plague swarm is truly destroyed only if it devours a good-aligned minor artifact or holy relic.
sources:
  - name: Bestiary 5
    desc: 192
```
# Description
There is no description for this monster.
# Source Link
[Archives of Nethys](https://aonprd.com/MythicMonsterDisplay.aspx?ItemName=Locust%20Plague%20Swarm)
```encounter-table
name: Mythic Locust Plague Swarm
creatures:
  - 1: Mythic Locust Plague Swarm
```
