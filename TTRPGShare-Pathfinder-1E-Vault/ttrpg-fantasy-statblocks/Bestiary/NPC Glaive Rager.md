---
statblock: inline
tags: monster
name: NPC Glaive Rager
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "NPC Codex"
Monster_CR: 7
name: NPC Glaive Rager
Monster_XP: 3200
race: Gnome
class: barbarian 8
alignment: CN
size: Small
type: humanoid
subtype: (gnome)
INI: +1
perception: +12
senses: low-light vision
AC: 17, touch 10, flat-footed 16 (armor +7, dex +1, rage -2, size +1)
HP: 105
HP_extra: 
HD: 8d12+48
saves: Fort +12, Ref +4, Will +5
saves_other: +4 vs. spells and spell-like or supernatural abilities, +2 vs. illusions
DR: 2/-
defensive_abilities: defensive training (+4 dodge bonus to AC vs. giants)
speed: 25 ft.
melee: mwk cold iron glaive +15/+10 (1d8+7/×3)
ranged: mwk composite longbow +11/+6 (1d6+5/×3)
special_attacks: +1 on attack rolls against goblinoid and reptilian humanoids, rage (21 rounds/day), rage powers (clear mind, knockback, quick reflexes, superstition +4)
space: 5 ft.
reach: 5 ft. (10 ft. with glaive)
tactics:
  - name: During Combat
    desc: The barbarian leads with trip maneuvers, seeking to trigger attacks of opportunity. He uses knockback to force creatures to advance on him again through the threatened area of his glaive.
  - name: Base Statistics
    desc: When not raging, the barbarian’s statistics are AC 19, touch 12, flat-footed 18; hp 89; Fort +10, Will +3; no bonus vs. spells and spell-like or supernatural abilities; Melee mwk cold iron glaive +13/+8 (1d8+4/×3) or kukri +12/+7 (1d3+3/18-20); Ranged mwk composite longbow +11/+6 (1d6+3/×3); Str 16, Con 16; CMB +10 (+14 trip); Skills Climb +6, Swim +8.
pf1e_stats: [20, 13, 20, 13, 10, 10]
BAB: 8
CMB: 12 (+16 trip)
CMD: 21 (23 vs. trip)
feats: Combat Expertise, Greater Trip, Improved Trip, Power Attack
skills: Acrobatics +5, Climb +8, Craft (ships) +5, Intimidate +5, Knowledge (nature) +10, Linguistics +3, Perception +12, Profession (sailor) +5, Survival +10, Swim +10
languages: Aquan, Auran, Common, Gnome, Sylvan
special_qualities: fast movement
gear:
  - name: combat
    desc: oil of magic weapon, potion of cure light wounds
  - name: other
    desc: +1 breastplate, kukri, masterwork cold iron glaive, masterwork composite longbow (+5 Str) with 20 arrows, belt of giant strength +2, cloak of resistance +1, climber’s kit, 50 gp
sources:
  - name: NPC Codex
    desc: 13
```
# Description
There is no description for this NPC.
# Source Link
[Archives of Nethys](https://aonprd.com/NPCDisplay.aspx?ItemName=Glaive%20Rager)
```encounter-table
name: NPC Glaive Rager
creatures:
  - 1: NPC Glaive Rager
```
