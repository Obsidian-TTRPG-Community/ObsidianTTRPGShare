---
statblock: inline
tags: monster
name: Half-Celestial (Unicorn)
---
```statblock
layout: Basic Pathfinder 1e Layout
source: "Pathfinder RPG Bestiary"
Monster_CR: 4
name: Half-Celestial (Unicorn)
Monster_XP: 1200
alignment: CG
size: Large
type: outsider
subtype: (native)
INI: +4
perception: +14
senses: darkvision, low-light vision, scent
aura: magic circle against evil
AC: 17, touch 13, flat-footed 13 (dex +4, natural +4, size -1, deflection vs. evil +2)
HP: 42
HD: 4d10+20
saves: Fort +9, Ref +8, Will +8
saves_other: +4 vs. poison
immune: charm, compulsion, disease, poison
resist: acid 10, cold 10, electricity 10
DR: 5/magic
SR: 15
speed: 60 ft., fly 120 ft. (good)
melee: gore +10 (1d8+6), 2 hooves +7 (1d3+3)
special_attacks: smite evil 1/day (+7 attack, +4 damage), powerful charge (gore, 2d8+12)
space: 10 ft.
reach: 5 ft.
pf1e_stats: [22, 19, 20, 13, 25, 26]
BAB: 4
CMB: 11
CMD: 25 (29 vs. trip)
feats: Multiattack, Weapon Focus (horn)
skills: Acrobatics +11, Fly +13, Knowledge (planes) +5, Perception +14, Sense Motive +14, Stealth +11, Survival +14
racial_modifiers:
- Survival 3
- Stealth 4
languages: Common, Sylvan
special_qualities: magical strike, wild empathy +18
ecology:
  - name: Environment
    desc: temperate forests
  - name: Organisation
    desc: solitary, mated pair, or blessing (3-6)
  - name: Treasure
    desc: none
spell-like_abilities:
  - name:
    desc: (CL 9)
  - name: At will
    desc: detect evil, light
  - name: 3/day
    desc: cure light wounds, protection from evil
  - name: 1/day
    desc: aid, bless, cure moderate wounds, greater teleport, neutralize poison
sources:
  - name: Pathfinder RPG Bestiary
    desc: 169
desc_short: A winged unicorn rises into the air on ivory pinions, a paragon of grace and beauty.
```
# Description
*Unicorn abilityDescriptionMost half-celestials are born of a mortal who loved a good outsider, but powerful holy magic can also create one.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=Half-Celestial%20(Unicorn))
```encounter-table
name: Half-Celestial (Unicorn)
creatures:
  - 1: Half-Celestial (Unicorn)
```
