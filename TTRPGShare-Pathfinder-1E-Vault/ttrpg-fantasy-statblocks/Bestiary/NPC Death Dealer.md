---
statblock: inline
tags: monster
name: NPC Death Dealer
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "NPC Codex"
Monster_CR: 18
name: NPC Death Dealer
Monster_XP: 153600
race: Half-orc
class: barbarian 19
alignment: CE
size: Medium
type: humanoid
subtype: (human, orc)
INI: +2
perception: +27
senses: darkvision
AC: 13, touch 10, flat-footed 11 (armor +3, dex +2, rage -2)
HP: 243
HP_extra: 
HD: 19d12+114
saves: Fort +20, Ref +12, Will +13
saves_other: +6 vs. spells and spell-like or supernatural abilities, +4 vs. enchantments
DR: 5/-
defensive_abilities: blink, improved uncanny dodge, indomitable will, orc ferocity, trap sense +6
speed: 40 ft., fly 60 ft. (good)
melee: +1 unholy ghost touch adamantine scythe +31/+26/+21/+16 (2d4+16/19-20/×4)
ranged: mwk composite longbow +22/+17/+12/+7 (1d8+10/×3)
special_attacks: greater rage (42 rounds/day), rage powers (clear mind, intimidating glare, knockback, mighty swing, strength surge +19, superstition +6, surprise accuracy +5, terrifying howl [DC 29], unexpected strike)
tactics:
  - name: Before Combat
    desc: The barbarian uses potions if expecting a major confrontation. His ring of blinking operates continuously.
  - name: During Combat
    desc: The barbarian begins combat with Dazzling Display and terrifying howl. Against a small group, he makes Flyby Attacks, using Lunge and potions of enlarge person to extend his reach and avoid attacks of opportunity, and Great Cleave against targets clustered together. In close combat, he uses intimidating glare along with Great Cleave attacks, using knockback to force enemies into close quarters. He relies on his ring of blinking to avoid most attacks, and with his tireless rage he exits and re-enters rage whenever it's convenient to regain the use rage powers he can use only once per rage.
  - name: Base Statistics
    desc: When not raging, the barbarian's statistics are AC 15, touch 12, flat-footed 13; hp 186; Fort +17, Will +10; no bonus vs. spells and spell-like or supernatural abilities, no bonus vs. enchantments; Melee +1 unholy ghost touch adamantine scythe +28/+23/+18/+13 (2d4+11/19-20/×4); Ranged mwk composite longbow +22/+17/+12/+7 (1d8+7/×3); Str 24, Con 14; CMB +26; CMD 38; Skills Climb +11, Intimidate +37, Swim +11.
pf1e_stats: [30, 14, 20, 8, 10, 12]
BAB: 19
CMB: 29
CMD: 39
feats: Cleave, Dazzling Display, Flyby Attack, Great Cleave, Improved Critical (scythe), Intimidating Prowess, Lunge, Power Attack, Shatter Defenses, Weapon Focus (scythe)
skills: Acrobatics +11, Climb +14, Fly +15, Intimidate +40, Perception +27, Survival +5, Swim +14
languages: Common, Orc
special_qualities: fast movement, orc blood, tireless rage, weapon familiarity
gear:
  - name: combat
    desc: oil of magic weapon, potion of bear’s endurance, potion of cat’s grace, potions of cure moderate wounds (2), potions of enlarge person (4), silversheen, tanglefoot bag
  - name: other
    desc: masterwork studded leather, +1 unholy ghost touch adamantine scythe, masterwork composite longbow (+10 Str) with 20 arrows, belt of giant strength +4, cloak of resistance +4, eyes of the eagle, handy haversack, mask of intimidation, ring of blinking, ring of feather falling, winged boots, 57 gp
sources:
  - name: NPC Codex
    desc: 24
```
# Description
These fearsome killers seek to feed their dark gods with the souls of those they slice apart with their powerful scythes-weapons powerful enough to cut through both flesh and spirit.
# Source Link
[Archives of Nethys](https://aonprd.com/NPCDisplay.aspx?ItemName=Death%20Dealer)
```encounter-table
name: NPC Death Dealer
creatures:
  - 1: NPC Death Dealer
```
