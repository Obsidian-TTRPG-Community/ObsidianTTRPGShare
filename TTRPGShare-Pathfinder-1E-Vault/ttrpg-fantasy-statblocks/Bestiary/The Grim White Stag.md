---
statblock: inline
tags: monster
name: The Grim White Stag
---
```statblock
layout: Basic Pathfinder 1e Layout
source:  "Inner Sea Gods"
Monster_CR: 15
name: The Grim White Stag
Monster_XP: 51200
alignment: LG
size: Colossal
type: outsider
subtype: (extraplanar, good, lawful)
INI: +2
perception: +22
senses: darkvision, low-light vision, detect evil
aura: protective aura
AC: 25, touch 5, flat-footed 22 (dex +2, dodge +1, natural +20, size -8, deflection vs. evil +4)
HP: 225
HP_extra: regeneration 5; regeneration weakness evil weapons and effects
HD: 18d10+126
saves: Fort +18, Ref +13, Will +7
saves_other: +4 vs. poison, +4 resistance vs. evil
immune: electricity, petrification
resist: cold 10, sonic 10
DR: 10/evil and silver
SR: 26
speed: 40 ft.
melee: gore +25 (4d6+14/19-20 plus push), 2 hooves +19 (2d8+7)
special_attacks: lay on hands (10/day, 9d6), push (gore, 10 ft.), trample (2d8+21, DC 33)
space: 30 ft.
reach: 20 ft.
pf1e_stats: [39, 14, 25, 10, 13, 12]
BAB: 18
CMB: 40 (+44 bull rush)
CMD: 53 (55 vs. bull rush, 57 vs. trip)
feats: Dodge, Endurance, Greater Bull Rush, Improved Bull Rush, Improved Critical (gore), Mobility, Power Attack, Run, Weapon Focus (gore)
skills: Acrobatics +11, Handle Animal +13, Intimidate +19, Knowledge (nature) +12, Perception +22, Sense Motive +13, Stealth +7, Survival +13, Swim +26
racial_modifiers:
- Stealth 8
languages: Celestial, Common, Druidic, Sylvan (can’t speak any language)
special_qualities: bugle, cascade of spears
ecology:
  - name: Environment
    desc: any land (Heaven)
  - name: Organisation
    desc: solitary
    desc: standard
special_abilities:
  - name: Bugle (Ex)
    desc: The Stag can make a distinctive call that can be heard for miles. All who worship Erastil immediately recognize the sound and know the direction and general distance to it.
  - name: Cascade of Spears (Su)
    desc: Once per day, the Stag can shed fragments of its antlers, creating up to 18 +1 shortspears, +1 spears, or +1 longspears in any combination (or substitute five +1 arrows or +1 crossbow bolts for each spear). These weapons retain their magic for 18 minutes, after which they become common weapons made of antler.
  - name: Heroes’ Feast (Sp)
    desc: The Stag lies down and dies to begin the “casting” of this ability, its body becoming a magical feast feeding up to 18 creatures. At the next sunrise, it returns to life with full hit points.
  - name: Protective Aura (Su)
    desc: This ability provides a +4 deflection bonus to AC and a +4 resistance bonus on saving throws, but only against attacks or effects from evil creatures.
spell-like_abilities:
  - name:
    desc: (CL 18; Concentration +19)
  - name: Constant
    desc: detect evil
  - name: 3/day
    desc: neutralize poison, remove disease
  - name: 1/month
    desc: heroes’ feast
sources:
  - name: Inner Sea Gods
    desc: 286
  - name: Pathfinder No. 32: Rivers Run Red
    desc: 82
desc_short: This gigantic stag-like being blurs the line between plant and beast, its horns branching like the limbs of some ancient, leaf less tree.
```
# Description
Erastil’s herald is rarely seen more than once in a given generation, and only when a settlement is in mortal danger. All good creatures of the forest recognize its near-divinity and come when they hear its bugling call. Other than its mighty summons, it does not speak to humanoids. It stands 60 feet tall and weighs approximately 140 tons.
# Source Link
[Archives of Nethys](https://aonprd.com/MonsterDisplay.aspx?ItemName=The%20Grim%20White%20Stag)
```encounter-table
name: The Grim White Stag
creatures:
  - 1: The Grim White Stag
```
